...
pub const TextMetrics = struct { fontWidth: f32, actualBoundingBoxLeft: f32, actualBoundingBoxRight: f32, actualBoundingBoxAscent: f32, actualBoundingBoxDescent: f32, fontBoundingBoxAscent: f32, fontBoundingBoxDescent: f32 };

pub const QuiteOkFont = struct { ascent: f32, descent: f32, glyphs: Array(Float32Array), characterMap: Map(u8, [2]f32) };
...
pub fn decodeFont(bytes: *Uint8Array) QuiteOkFont {
    ...
    var idx: u32 = 3;
    const ascent = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const descent = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const minW = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const maxW = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const pXMin = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const pXMax = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const pYMin = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    const pYMax = decodeFloat(bytes, idx);
    idx += QOF_FLOAT_SZ;
    ...
    var i: u32 = idx + 2;
    while (i < idx + 2 + charMapTableLen) : (i += 1) {
        characterMap.set(bytes.get(i), [2]f32{ As.f32(bytes.get(i + charMapTableLen)), minW + As.f32(bytes.get(i + charMapTableLen * 2)) / 255 * (maxW - minW) });
    }
    ...
    while (cidx < startI + endOff) {
        const seg = [_]u8{ bytes.get(cidx) >> 6, (bytes.get(cidx) >> 4) & 3, (bytes.get(cidx) >> 2) & 3, bytes.get(cidx) & 3 };
        var has3 = false;
        var k: usize = 0;
        while (k < seg.len) : (k += 1) {
            if (seg[k] == 3) {
    ...
    return QuiteOkFont{ .ascent = ascent, .descent = descent, .glyphs = glyphs, .characterMap = characterMap };
}

pub fn measureText(txt: *String, myFont: QuiteOkFont, sz: f32) ?TextMetrics {
    const VERT = 0;
    const QUAD = 1;
    const HOLE = 2;
    ...
    return TextMetrics{ .fontWidth = fontWidth, .actualBoundingBoxLeft = offX, .actualBoundingBoxRight = fontWidth - endOffX, .actualBoundingBoxAscent = -offY, .actualBoundingBoxDescent = offEndY, .fontBoundingBoxAscent = ascent, .fontBoundingBoxDescent = descent };
}
