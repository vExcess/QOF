...
pub const Math = struct {
    ...
    pub fn pow(x: anytype, y: anytype) @TypeOf(x, y) {
        if (@TypeOf(x, y) == comptime_int) {
            return std.math.pow(i64, As.i64(x), As.i64(y));
        }
...
pub fn Array(comptime T: type) type {
    return struct {
        ...
        pub fn using(buffer: []T) Self {
            return Self{ .buffer = buffer, .len = As.u32(buffer.len) };
        }
        ...
        pub fn slice(self: *const Self, start_: u32, end_: anytype) Self {
            ...
            return Self{ .buffer = self.buffer[start..end] };
        }
    };
}
...
pub fn Map(comptime KeyType: type, comptime ValueType: type) type {
    return struct {
        pub const Entry = struct { key: ?KeyType, value: ValueType, hash: u32 };
        ...
        pub fn alloc() Self {
            ...
            while (i < buckets.len) : (i += 1) {
                buckets[i] = Entry{ .key = null, .value = undefined, .hash = undefined };
            }
            const keys = Array(KeyType).alloc(0);
            return Self{ .buckets = buckets, .keys = keys };
        }
        ...
        pub fn grow(self: *Self) void {
            ...
            while (i < newBuckets.len) : (i += 1) {
                newBuckets[i] = Entry{ .key = null, .value = undefined, .hash = undefined };
            }
        ...
        pub fn setPreHashed(self: *Self, key: KeyType, hash: u32, value: ValueType) void {
            ...
            if (isEmpty or Self.keyEql(bucket.key.?, key)) {
                ...
                self.buckets[idx] = Entry{ .key = key, .value = value, .hash = hash };
                if (!isEmpty) {
                    self.size += 1;
                }
            } else {
...
pub const String = struct {
    ...
    pub fn alloc(capacity: u32) String {
        ...
        return String{ .viewStart = 0, .viewEnd = 0, .bytes = bytes, .isSlice = false };
    }
    ...
    pub fn using(bytes: Uint8Array) String {
        return String{ .viewStart = 0, .viewEnd = bytes.len, .bytes = bytes, .isSlice = false };
    }
    ...
    pub fn usingRawString(_cstring: [:0]const u8) String {
        ...
        bytes.buffer.len += 1;
        return String{ .viewStart = 0, .viewEnd = As.u32(_cstring.len), .bytes = bytes, .isSlice = false };
    }
    ...
    pub fn slice(self: *const String, start_: u32, end_: anytype) String {
        ...
        return String{ .viewStart = self.viewStart + start, .viewEnd = self.viewStart + end, .bytes = self.bytes, .isSlice = true };
    }
    ...
    pub fn trimStart(self: *const String) String {
        ...
        return String{ .viewStart = start, .viewEnd = end, .bytes = self.bytes, .isSlice = true };
    }
